...
def correlate():
    target_chips = glob.glob('/home/bruce/data/ATR/new_samplesets/unscaled_1to2/chips20x40/targets/' + '*.mat')
    clutter_chips = glob.glob('/home/bruce/data/ATR/new_samplesets/unscaled_1to2/chips20x40/clutter/' + '*.mat')
...
def make_basis():
    ...
    sdelta = sdelta[skip:]
    sphi = sphi[:,skip:]
    abc = np.matmul(sphi,npinv(sdelta *np.eye(130)))
    ...
    n2 = S2.shape[1]
    print(n1,n2)
    np.save('data/target_filters',S1)
    np.save('data/clutter_filters',S2)
